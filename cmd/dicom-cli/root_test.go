...
func TestExecuteHelpExplainsEveryProjectCommand(t *testing.T) {
	tests := []struct {
		...
		{name: "convert", args: []string{"convert", "--help"}, want: "DICOM input can be exported"},
		...
		{name: "convert xml", args: []string{"convert", "xml", "--help"}, want: "--input is required"},
		{name: "convert pixeldata", args: []string{"convert", "pixeldata", "--help"}, want: "--input is required"},
