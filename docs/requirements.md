...
命令语言不得通过额外全局参数或环境变量覆盖。运行配置根字段 `language` 只接受
`en` 和 `zh-CN`，缺省为 `en`；`lang <en|zh-CN>` 用于持久修改所选配置。帮助、
文本结果、进度和本工具生成的诊断均随所选运行配置切换。命令/flag 名、JSON 字段和值、退出码、
规则 DSL、DICOM Tag 和 UID 保持稳定。配置不存在、无法读取或语言值非法时，诊断以英文输出。
