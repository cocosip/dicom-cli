...
- [ ] **P2.1 建立合成 DICOM fixture 工厂**：依赖 P0.1；完成条件：提供无真实患者信息的单帧、多帧、序列、UID 引用和损坏文件 fixture，均可被内部包复用。
- [ ] **P2.2 封装 DICOM 文件读写**：依赖 P2.1；完成条件：基于 go-dicom 读写文件元信息和数据集，读写测试证明源文件未被改写。
- [ ] **P2.3 实现 Tag 路径读取**：依赖 P2.1、P2.2；完成条件：支持关键字、十六进制和嵌套序列索引，正反例测试通过。
- [ ] **P2.4 实现 Tag VR 判定**：依赖 P2.3；完成条件：标准 Tag 可由字典取得 VR，私有/未知 Tag 缺显式 VR 时失败。
- [ ] **P2.5 实现 UID 生成**：依赖 P2.1；完成条件：支持 `2.25.<UUID 十进制>` 和机构 UID 根，格式与唯一性测试通过。
- [ ] **P2.6 实现单次调用 UID 映射**：依赖 P2.5；完成条件：同一原 UID 一致、不同原 UID 不合并，Study/Series/SOP 与引用 UID 测试通过。
- [ ] **P2.7 实现目录扫描**：依赖 P0.6、P1.7；完成条件：支持当前层、递归、目录链接排除、文件链接处理与筛选跳过。
- [ ] **P2.8 实现安全输出路径**：依赖 P2.7；完成条件：支持默认目录、保留层级、平铺、重名序号，并拒绝输入同路径输出。
- [ ] **P2.9 实现输出适配**：依赖 P0.5、P2.8；完成条件：文本、JSON、单二进制 stdout 与目录输出均可用，多结果二进制 stdout 被拒绝。
- [ ] **P2.10 实现批处理执行器**：依赖 P2.7、P2.9；完成条件：统计 scanned/processed/skipped/failed，保留筛选跳过原因，支持继续处理与 fail-fast。
- [ ] **P2.11 验收 P2**：依赖 P2.1-P2.10；完成条件：`go test ./internal/dicom ./internal/files ./internal/output ./internal/app ./internal/testutil` 成功。
