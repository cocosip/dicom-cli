...
var englishCommands = map[string]CommandText{
	...
	"dicom-cli convert json":         {Short: "Export DICOM metadata as JSON", Long: "Export DICOM metadata as JSON. --input is required and accepts one DICOM file or directory; positional input is not supported. PixelData is summarized by default; use --include-pixel-data only when the full pixel bytes are required."},
...
var chineseCommandLong = map[string]string{
	...
	"dicom-cli convert json":         "将 DICOM 元数据导出为 JSON。--input 为必填项，且只接受一个 DICOM 文件或目录；不支持位置输入参数。默认仅汇总 PixelData；只有需要完整像素字节时才使用 --include-pixel-data。",
