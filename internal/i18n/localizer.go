...
var englishCommands = map[string]CommandText{
	...
	"dicom-cli convert":              {Short: "Export DICOM images, metadata, and PixelData", Long: "DICOM input can be exported as rendered image frames, metadata JSON or XML, or raw PixelData bytes. Select the image, json, xml, or pixeldata subcommand; conversion never rewrites the source DICOM file."},
	"dicom-cli convert image":        {Short: "Export DICOM pixel data as PNG or JPEG", Long: "Export DICOM pixel data as PNG or JPEG. --input is required and accepts one DICOM file or directory; positional input is not supported. Frame numbers start at 1; without --frame or --all-frames, the first frame is exported. Binary stdout requires exactly one result."},
	"dicom-cli convert json":         {Short: "Export DICOM metadata as JSON", Long: "Export DICOM metadata as JSON without PixelData. --input is required and accepts one DICOM file or directory; positional input is not supported. Use convert pixeldata to export raw pixel bytes."},
	"dicom-cli convert xml":          {Short: "Export DICOM metadata as XML", Long: "Export DICOM metadata as Native DICOM Model XML without PixelData. --input is required and accepts one DICOM file or directory; positional input is not supported. Use convert pixeldata to export raw pixel bytes."},
	"dicom-cli convert pixeldata":    {Short: "Export raw DICOM PixelData bytes", Long: "Export stored PixelData frame payloads to a binary file. --input is required and accepts one DICOM file or directory; positional input is not supported. Frame payloads are concatenated in order without decoding, transcoding, DICOM element headers, Basic Offset Table, or fragment item headers."},
	...
	"dicom-cli transcode":            {Short: "Re-encode DICOM transfer syntaxes", Long: "Re-encode DICOM transfer syntaxes.\n\nProvide exactly one input path with --input (recommended) or as the optional positional argument. The input can be a DICOM file or a directory. With a file input, --output is the new DICOM file path; with a directory input, --output is the output directory. When --output is omitted, results are written below the transcode directory in the current working directory. --to accepts a transfer syntax standard name, short name, or UID. Run `dicom-cli transcode formats` to list values available in this binary. --to is required, and source files are never overwritten."},
...
var chineseCommandShort = map[string]string{
	...
	"dicom-cli convert":              "导出 DICOM 图像、元数据和 PixelData",
	"dicom-cli convert image":        "将 DICOM 像素数据导出为 PNG 或 JPEG",
	"dicom-cli convert json":         "将 DICOM 元数据导出为 JSON",
	"dicom-cli convert xml":          "将 DICOM 元数据导出为 XML",
	"dicom-cli convert pixeldata":    "导出原始 DICOM PixelData 字节",
...
var chineseCommandLong = map[string]string{
	...
	"dicom-cli convert":              "将 DICOM 输入导出为渲染图像帧、JSON/XML 元数据或原始 PixelData 字节。选择 image、json、xml 或 pixeldata 子命令；转换不会改写源 DICOM 文件。",
	"dicom-cli convert image":        "将 DICOM 像素数据导出为 PNG 或 JPEG。--input 为必填项，且只接受一个 DICOM 文件或目录；不支持位置输入参数。帧号从 1 开始；未提供 --frame 或 --all-frames 时导出首帧。二进制标准输出要求恰好一个结果。",
	"dicom-cli convert json":         "将不含 PixelData 的 DICOM 元数据导出为 JSON。--input 为必填项，且只接受一个 DICOM 文件或目录；不支持位置输入参数。需要原始像素字节时使用 convert pixeldata。",
	"dicom-cli convert xml":          "将不含 PixelData 的 DICOM 元数据导出为 Native DICOM Model XML。--input 为必填项，且只接受一个 DICOM 文件或目录；不支持位置输入参数。需要原始像素字节时使用 convert pixeldata。",
	"dicom-cli convert pixeldata":    "将存储的 PixelData 帧载荷导出为二进制文件。--input 为必填项，且只接受一个 DICOM 文件或目录；不支持位置输入参数。按顺序拼接帧载荷，不解码、不转码，也不包含 DICOM 元素头、Basic Offset Table 或 fragment Item 头。",
	...
	"dicom-cli transcode":            "重编码 DICOM 传输语法。使用 --input（推荐）或可选位置参数提供一个输入路径，二者只能传其一；输入可为 DICOM 文件或目录。文件输入时 --output 是新的 DICOM 文件路径，目录输入时 --output 是输出目录。未提供 --output 时，结果写入当前工作目录下的 transcode 目录。--to 接受传输语法标准名称、短名称或标准 UID；运行 dicom-cli transcode formats 可列出当前二进制可用值。只有 --to 为必填，且绝不覆盖源文件。",
...
var chineseInspectLabels = map[string]string{
	...
	"[Pixel]":                           "[像素]",
	"[Elements]":                        "[数据元素]",
	"[File Meta Elements]":              "[文件元信息元素]",
	"[Selected Tags]":                   "[选定 Tag]",
	"  Path:":                           "  路径：",
	"  Transfer Syntax:":                "  传输语法：",
	"  Transfer Syntax UID:":            "  传输语法 UID：",
	"  Transfer Syntax Name:":           "  传输语法名称：",
	"  VR Encoding:":                    "  VR 编码：",
	"  Byte Order:":                     "  字节序：",
	"  Encapsulated:":                   "  封装：",
	"  Lossy:":                          "  有损：",
	"  Lossy Compression Method:":       "  有损压缩方法：",
	"  Deflated:":                       "  Deflate：",
	"  Retired:":                        "  已废弃：",
	"  Media Storage SOP Class UID:":    "  媒体存储 SOP Class UID：",
	"  Media Storage SOP Instance UID:": "  媒体存储 SOP Instance UID：",
	"  Implementation Class UID:":       "  实现类 UID：",
	"  Implementation Version Name:":    "  实现版本名称：",
	"  Source Application AE Title:":    "  源应用 AE Title：",
	"  Specific Character Set:":         "  特定字符集：",
	"  Manufacturer:":                   "  制造商：",
	"  Model:":                          "  型号：",
	"  Station:":                        "  工作站：",
	...
	"  Rescale Intercept:":              "  重定标截距：",
	"  Rescale Slope:":                  "  重定标斜率：",
	"  Rescale Type:":                   "  重定标类型：",
	"  VOI LUT Function:":               "  VOI LUT 函数：",
	"  Lossy Image Compression:":        "  有损图像压缩：",
	"  Lossy Image Compression Ratio:":  "  有损图像压缩比：",
	"  Lossy Image Compression Method:": "  有损图像压缩方法：",
}
